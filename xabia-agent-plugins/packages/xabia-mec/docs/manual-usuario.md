...
1. Instale y active **Xabia Agent Core** (`v1.0.57` o superior recomendado).
2. Instale y active el ZIP **Xabia MEC** desde **Plugins → Añadir nuevo → Subir plugin**.
...
| **Precio mostrado** (p. ej. 49€ / año) | Texto orientativo del catálogo; el precio final lo confirma **Polar** en el checkout. |
...
Si no va a instalar **Xabia MEC** en el sitio del chat, puede usar **Base de Datos Externa (SQL Remoto) → Usar preset MEC remoto** como alternativa. El modo addon remoto es el recomendado cuando la licencia y el plugin Xabia MEC están activos.
...
Cuando el proyecto usa el motor de **reservas MEC** y el evento tiene **booking** habilitado, la respuesta puede incluir un **enlace o botón** que abre la página del evento (con ancla `#book` por defecto, personalizable por desarrolladores con filtros WordPress).

En catálogos **MEC remotos** sin MEC local en el WordPress del chat, Xabia no genera acciones `[ACTION:BOOK]` locales porque no puede verificar el formulario de reserva de ese sitio. En su lugar, dirige al visitante al campo **Link** del evento remoto.
...
## Parte 10 — Opciones solo para desarrolladores / integradores (referencia)

No hace falta tocarlas en un uso normal; existen para flexibilidad:
...
| `xabia_addons_registry` | Permite al desarrollador sustituir URLs o metadatos del catálogo Polar. |

---

## Parte 11 — Problemas frecuentes y checklist
...
| **403 en federación** | Licencia MEC, clave **X-Xabia-Key**, o sesión de administrador. |
| **Aviso «requiere Core»** al activar MEC | Active **xabia-intelligence.php** primero. |
